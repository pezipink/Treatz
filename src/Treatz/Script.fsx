...
type QuadTreeLocation =
    | TopRight    
    | BottomRight
    | BottomLeft    
    | TopLeft    
        
let getQuadrant itemBounds outerQuadrantBounds =
...
let findNeighboursf pred itemBounds maxBounds tree =
    let rec aux outerBounds = function
        ...
        | Branch(data,TR,BR,BL,TL) -> 
            ...
            | Some _ -> 
                ...
                [ yield! collectDataf TR
                  yield! collectDataf BR
                  yield! collectDataf BL
                  yield! collectDataf TL ]
            | None -> 
...
let items = [|for i in 1..30000 ->  { x = rnd.Next(1,800); y = rnd.Next(1,600); width = rnd.Next(1,10); height = rnd.Next(1,10) } |]
let bounds = { x = 0; y = 0; width = 800; height = 600}
let tree = create (List.ofArray items) id 6 15 bounds
...
let fp = 
     items |> Array.Parallel.iter(fun i -> findNeighboursf ((=)i) i bounds tree |> ignore)

let ip = 
    items |> Array.Parallel.iter(fun i -> findNeighbours ((=)i) i bounds tree |> ignore)
